...
} from './cash-to-close-calculator.js';
...
function furniture(el, d) {
  if (d.footer) {
    ...
    const split = d.footerTheme === 'split';
    const f = document.createElement('div');
    f.className = 'slide-footer';
    f.innerHTML = `
      <div class="footer-logo"><img src="${split ? LOGO.onLight : dark ? LOGO.onDark : LOGO.onLight}" alt="Mountain State Financial Group"></div>
    ...
    if (split) el.dataset.footerTheme = 'split';
  }
  const lines = [];
...
const layouts = {
  ...
  statement(el, d) {
    el.innerHTML = `
      <div class="statement">
        <span class="eyebrow build">${d.eyebrow}</span>
        <h1 class="statement-text build">${d.headline}</h1>
        <div class="accent-bar build"></div>
  ...
  closePlan(el, d) {
    el.innerHTML = `
      <div class="close-plan">
        <div class="close-plan-copy">
          <span class="eyebrow build">${d.eyebrow}</span>
          <h1 class="headline build">${d.headline}</h1>
          <div class="accent-bar build"></div>
          <p class="close-presenter build">${P.name}</p>
          <p class="close-role build">${[P.title, P.nmls].filter(Boolean).join(' · ')}</p>
          <ul class="contact build">
            ${contactList([
              { label: 'Call', value: P.phone, href: telHref(P.phone) },
              { label: 'Email', value: P.email, href: mailHref(P.email) },
        ...
        <div class="closing-questions build">
          <p class="closing-label">Leave with three answers</p>
          ${d.questions.map((question, index) => `
            <div><span>0${index + 1}</span><p>${question}</p></div>`).join('')}
        </div>
      </div>`;
  },
...
function clearBuildTimers() {
  buildPlayback.timers.forEach(timer => clearTimeout(timer));
...
function broadcastAnimationState() {
  ...
  channel.postMessage({
    ...
    current: buildPlayback.revealed,
    total: buildPlayback.items.length,
    playing: buildPlayback.playing,
  });
}
...
function stepBuild(delta) {
  clearBuildTimers();
  buildPlayback.playing = false;
  revealBuilds(buildPlayback.revealed + delta);
}

function playBuild() {
  clearBuildTimers();
  const total = buildPlayback.items.length;
  if (!total) {
    buildPlayback.playing = false;
    broadcastAnimationState();
    return;
  }
  if (buildPlayback.revealed >= total) revealBuilds(0);

  const start = buildPlayback.revealed;
  buildPlayback.playing = true;
  broadcastAnimationState();
  buildPlayback.items.slice(start).forEach((item, offset) => {
    ...
    buildPlayback.timers.push(timer);
  });
}
...
function broadcastCashToCloseState() {
  if (channel) channel.postMessage({ type: 'cash-to-close-state', visible: isCashToCloseVisible() });
}
function initChannel() {
  if (!('BroadcastChannel' in window) || PREVIEW) return;   // preview instance stays silent
  channel = new BroadcastChannel(DECK_CHANNEL_NAME);
  channel.onmessage = e => {
    const m = e.data;
    ...
    if (m.type === 'animation-pause') pauseBuild();
    if (m.type === 'open') openModal(m.id);
    if (m.type === 'open-media') openMedia(m.id);
    if (m.type === 'cash-to-close-visibility' && typeof m.visible === 'boolean') {
      setCashToCloseVisible(m.visible);
    }
    if (m.type === 'hello') {
      broadcast();
      channel.postMessage({ type: 'navstate', hidden: navHidden });
      broadcastCashToCloseState();
      broadcastAnimationState();
    }
...
export function initDeck() {
  ...
  initModal();
  if (!PREVIEW) {
    initCashToCloseCalculator({ onVisibilityChange: broadcastCashToCloseState });
