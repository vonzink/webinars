...
const mythGuessNotes = {
  20: "That is the answer most buyers hear first. It can be a strong strategy, but it is not the entrance fee.",
  10: "Closer for many buyers, but still not the whole market. The real answer depends on program fit.",
  5: "Often closer to the starting point. The next question is whether the payment and cash-to-close still feel healthy.",
  unsure: "That is the honest answer. The right path depends on buyer, property, program, timing, and verified eligibility."
};
...
function showSlide(index) {
  ...
  slides.forEach((slide, slideNumber) => {
    ...
    if (slideNumber === currentSlide && slide.dataset.spaceReveal) {
      ...
      slide.querySelectorAll("[data-reveal-item]").forEach((item) => {
...
function revealCurrentSlide() {
  ...
  if (!activeSlide.dataset.spaceReveal) {
    return false;
  }
  const revealItems = Array.from(activeSlide.querySelectorAll("[data-reveal-item]"));
  const nextItem = revealItems.find((item) => !item.classList.contains("revealed"));
  if (!nextItem) return false;

  nextItem.classList.add("revealed");
  nextItem.setAttribute("aria-hidden", "false");
  if (revealItems.every((item) => item.classList.contains("revealed"))) {
...
function estimateAprFromFees(principal, noteRate, fees, months = 360) {
  const payment = monthlyPayment(principal, noteRate, months);
  const amountFinanced = Math.max(1, principal - fees);
  let low = 0.01;
  let high = 20;

  for (let i = 0; i < 60; i += 1) {
    const mid = (low + high) / 2;
    const candidate = monthlyPayment(amountFinanced, mid, months);
...
function updateAprFeeDemo() {
  const feeInput = document.getElementById("aprFeeInput");
  if (!feeInput) return;

  const fees = value("aprFeeInput");
  const apr = estimateAprFromFees(aprFeeDemo.loanAmount, aprFeeDemo.noteRate, fees, aprFeeDemo.months);
  const delta = Math.max(0, apr - aprFeeDemo.noteRate);

  write("aprNoteRateOut", `${aprFeeDemo.noteRate.toFixed(3)}%`);
  write("aprLoanOut", `${money.format(aprFeeDemo.loanAmount)} loan`);
  write("aprFeeOut", money.format(fees));
  write("aprLiveOut", `${apr.toFixed(3)}%`);
  write("aprDeltaOut", `Fees add ${delta.toFixed(3)}%`);
}
...
document.querySelectorAll("[data-component]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    document.querySelectorAll("[data-component-segment]").forEach((segment) => {
      segment.classList.toggle("active", segment.dataset.componentSegment === component);
    });
    write("componentNote", componentNotes[component]);
  });
});
...
document.querySelectorAll("[data-myth-guess]").forEach((button) => {
  button.addEventListener("click", () => {
    const guess = button.dataset.mythGuess;
    setActive(button, "[data-myth-guess]");
    document.querySelectorAll("[data-myth-guess]").forEach((item) => {
      item.setAttribute("aria-pressed", String(item === button));
    });
    button.closest(".myth-slide").classList.add("revealed");
    write("mythAnswerOut", mythGuessNotes[guess]);
  });
});
...
document.querySelectorAll("[data-apr-popout]").forEach((button) => {
  button.addEventListener("click", () => {
    const popout = document.getElementById("aprPopout");
    const isOpen = popout.classList.toggle("open");
...
const draggableLogo = document.querySelector("[data-draggable-logo]");
...
function deckScaleValue() {
  return Number.parseFloat(getComputedStyle(deck).getPropertyValue("--deck-scale")) || 1;
}

if (draggableLogo) {
  draggableLogo.addEventListener("pointerdown", (event) => {
    if (event.target.closest("a")) return;
    event.preventDefault();
    const scale = deckScaleValue();
    const startX = Number.parseFloat(draggableLogo.style.getPropertyValue("--logo-drag-x")) || 0;
    const startY = Number.parseFloat(draggableLogo.style.getPropertyValue("--logo-drag-y")) || 0;
    logoDragState = {
      pointerX: event.clientX / scale,
      pointerY: event.clientY / scale,
      startX,
      startY
    };
    draggableLogo.classList.add("dragging");
  });
}

window.addEventListener("pointermove", (event) => {
  if (!logoDragState || !draggableLogo) return;
  const scale = deckScaleValue();
  const nextX = logoDragState.startX + event.clientX / scale - logoDragState.pointerX;
  const nextY = logoDragState.startY + event.clientY / scale - logoDragState.pointerY;
  draggableLogo.style.setProperty("--logo-drag-x", `${clamp(nextX, -180, 380)}px`);
  draggableLogo.style.setProperty("--logo-drag-y", `${clamp(nextY, -120, 120)}px`);
});

window.addEventListener("pointerup", () => {
  if (!logoDragState || !draggableLogo) return;
  logoDragState = null;
  draggableLogo.classList.remove("dragging");
});
